...
@click.option("--type", "tx_type", help="Filter by transaction type (comma-separated)")
@click.option("--method", help="Filter by method name (comma-separated)")
@click.option(
    "--format",
    "-f",
    "output_format",
...
def list(ctx, filter_type, tx_type, method, output_format):
    ...
    try:
        with BlockScoutClient(config.base_url, config.timeout) as client:
            ...
            output = format_output(limited_results, format_type, "Recent Transactions")
    ...
    try:
        with BlockScoutClient(config.base_url, config.timeout) as client:
            ...
            output = format_output(
                limited_results, format_type, f"Token Transfers for {tx_hash}"
            )
            console.print(output)
